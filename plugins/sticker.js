  ...
  let weem2 = `\t「\t🍃\t」\t`
   ...
   try {
      ...
      if (/stickerMessage/g.test(mime)) {
         ...
         let img = await q.download?.()
         stiker = await createSticker(img, false, packnames || weem2, weem || m.name)
         ...
         let img = await q.download?.()
         stiker = await createSticker(img, false, packnames || weem2, weem || m.name, 30)
      } else if (/viewOnceMessageV2/g.test(mime)) {
         let img = await q.download?.()
         stiker = await sticker(img, false, packnames || weem2, weem || m.name)
      } else if (args[0] && isUrl(args[0])) {
         stiker = await createSticker(false, args[0], packnames || weem2, weem || m.name, 30)
         ...
         if (firstEmoji) {
            ...
            stiker = await createSticker(await (await conn.getFile(emj)).data, false, packnames || weem2, weem || m.name, 30)
         } else {
